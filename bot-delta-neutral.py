...
if len(sys.argv) != 5:
    print(f" \nUsage: $ python3 bot-delta-neutral.py [pair] [total_usdt_investment] [stop.delay.minutes] [tlgrm.msg.title]\n ")
    sys.exit(1)
print(" ")
...
usd_to_short = round(howmuchusd*(1/3),4)
howmuchusd = round(howmuchusd*(2/3),4)
...
for n in echanges_str:
    ...
    if float(bal) < float((howmuchusd)/len(echanges)):
        printandtelegram(f'[{time.strftime("%H:%M:%S", time.gmtime(time.time()))}] Not enough balance on {n}. Need {round(float((howmuchusd)/len(echanges))-float(bal),3)} USDT more. Current balance on {n}: {round(bal,3)} USDT')
        s=1
bal_kuc_futures = ex["kucoinfutures"].fetch_balance()
bal_kuc_futures = float(bal_kuc_futures["total"]["USDT"])
if bal_kuc_futures < usd_to_short:
    printandtelegram(f"[{time.strftime('%H:%M:%S', time.gmtime(time.time()))}] Not enough balance on kucoin-futures. Need {round(usd_to_short-bal_kuc_futures,3)} USDT more. Current balance: {round(bal_kuc_futures,3)} USDT")
if s==1:
    sys.exit(1)
...
totalDollar1 = usd['binance'] + usd['kucoin'] + usd['okx']
...
if balance_usdt_kucoinfutures['total']['USDT'] < usd_to_short:
    ex['kucoin'].transfer('USDT',amount=round(usd_to_short-balance_usdt_kucoinfutures['total']['USDT'],3),fromAccount='spot',toAccount='future')
    printandtelegram(f"[{time.strftime('%H:%M:%S', time.gmtime(time.time()))}] {round(usd_to_short,3)} USDT successfully transferred from kucoin spot (trading) to kucoin futures.")
ex['kucoinfutures'].createMarketSellOrder(kucoinPair+":USDT",round(usd_to_short/average_first_buy_price,3),{'leverage':1})
printandtelegram(f"[{time.strftime('%H:%M:%S', time.gmtime(time.time()))}] Delta-neutral short for {howmuchusd*(1/3)*average_first_buy_price} {kucoinPair[:len(kucoinPair)-5]} successfully sent to ex['kucoin']. Waiting 120 seconds for fill...")
delta_neutral_order_fill_timeout = time.time() + 5000
delta_neutral_order = ex['kucoinfutures'].fetchOpenOrders(symbol=kucoinPair+":USDT")
while delta_neutral_order != []:
    time.sleep(1)
    delta_neutral_order = ex['kucoinfutures'].fetchOpenOrders(symbol=kucoinPair[:-5]+":USDT")
    if time.time() > delta_neutral_order_fill_timeout:
            ex['kucoinfutures'].cancelOrder(id=delta_neutral_order[0]['info']['id'])
            printandtelegram(f"{[{time.strftime('%H:%M:%S', time.gmtime(time.time()))}]} Delta-neutral short order not filled after 5000 seconds. Exiting.")
            sys.exit(1)
            ...
            if max_bid_ex != min_ask_ex and profit_with_fees_usd > float(criteria_usd) and profit_with_fees_pct > float(criteria_pct) and prec_ask_price != min_ask_price and prec_bid_price != max_bid_price and usd[min_ask_ex] >= crypto_per_transaction * min_ask_price * (1+fees[min_ask_ex]['give']):
                ...
                if sell_order != [] and buy_order == []:
                    ...
                    try:
                        ...
                        ex[min_ask_ex].createMarketSellOrder(kucoinPair,None,{"quoteOrderQty":float(last_opposite_orders[len(last_opposite_orders)-1]['cost'])})
                    except Exception as e:
                        sys.exit(1)
            else:
